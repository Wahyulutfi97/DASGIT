...
@frappe.whitelist()
def get_product_lazada():
	lazada_setting = frappe.get_doc("Lazada Store", "GembulCubet")
	client =  LazopClient("https://api.lazada.co.id/rest", lazada_setting.api_key, lazada_setting.api_secret)
	request = LazopRequest('/products/get','GET')
	request.add_api_param('filter', 'live')
	response = client.execute(request, lazada_setting.access_token)
	print(response.type)
	print(response.body)
...
def generate_access_token():
	client =  LazopClient("https://auth.lazada.com/rest", "123109" , "3hqyHutvkeIJlSDrgSeHk8c9qvd4Bdj7")
	request = LazopRequest("/auth/token/create",'GET')
	request.add_api_param('code', "0_123109_O4nl092PEHHjpo7Z2rqfIkg11049")

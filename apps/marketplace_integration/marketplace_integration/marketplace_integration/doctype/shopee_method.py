...
@frappe.whitelist()
def shopee_order_tiap_jam():
	# ambil semua store yang enable
	get_store = frappe.db.sql("""
		SELECT sss.`name` FROM `tabShopee Store Setting` sss
	...
	if get_store :
		for gs in get_store :
			...
			enqueue("marketplace_integration.marketplace_integration.doctype.shopee_method.enqueue_shopee_order", date=today_date, store_setting = gs[0])

@frappe.whitelist()
def enqueue_shopee_order(date=None, store_setting=None):
	count = 0
	time_from = 0
	time_to = 0
	curr_date = date
	...
	# check setting
	shopee_setting = frappe.get_doc("Shopee Store Setting", store_setting)
				...
				if result["orders"] :
					...
					for i in result["orders"] :
						...
						if get_morder :
							...
							if str(i["order_status"]=="UNPAID" or i["order_status"]=="READY_TO_SHIP" or i["order_status"]=="CANCELLED" or i["order_status"]=="COMPLETED" or i["order_status"]=="IN_CANCEL" or i["order_status"]=="TO_RETURN") :
								...
								if get_marketplace_order.order_status != i["order_status"] :
									...
									get_docu.store_name = shopee_setting.shop_name
						...
						# tidak ada
							...
							new_docu.store_name = shopee_setting.shop_name
...
def generate_url_untuk_order_id():
	...
	partner_key = "9d56537cd047d03513cddede1fcd590a41de4ad94ac977e8da3c3ba2f5f25e60"
	...
	if shopee_setting.enable_sync :
		...
		for looping in range(25) :
			if temp_continue == False :
				...
				result = shopeeclient.order.get_order_list(create_time_from = time_from, create_time_to = time_to, pagination_entries_per_page = pagination_entry, pagination_offset = pagination_offset)
				# frappe.throw(str(result))
...
def get_shopee_logistic():
	...
	result = shopeeclient.logistic.get_logistics()
...
def get_shopee_item_category_child():
	...
	result = shopeeclient.item.get_categories(language = "en")
			...
			if doc.shopee_item_id :
				...
				if shopee_setting.enable_sync :
					...
					if shopee_setting.enable_sync :
						...
						result = shopeeclient.item.add(product_data = array_data)

...
@frappe.whitelist()
def tokopedia_order_tiap_jam():
	# ambil semua store yang enable
	get_store = frappe.db.sql("""
		SELECT sss.`name` FROM `tabTokopedia Store Setting` sss
	...
	if get_store :
		for gs in get_store :
			...
			enqueue("marketplace_integration.marketplace_integration.doctype.tokopedia_method.enqueue_tokopedia_order", date=today_date, store_setting = gs[0])
...
@frappe.whitelist()
def enqueue_tokopedia_order(date=None, store_setting=None):
	# frappe.msgprint("coba")

	# check setting
	tokopedia_setting = frappe.get_doc("Tokopedia Store Setting", store_setting)
	
	today = date
	# back_date = str(add_days(today, -1))
	# start_date = str(add_days(today, 1))
	from_date = str(today)+ " 00:00:00"
	to_date = str(today)+ " 23:59:59"
	...
	# url = "https://fs.tokopedia.net/v1/fs/"+str(tokopedia_setting.app_id)+"/fulfillment_order"
	url = "https://fs.tokopedia.net/v2/order/list?fs_id="+str(tokopedia_setting.app_id)+"&shop_id="+str(tokopedia_setting.shop_id)+"&from_date="+ strtime_from +"&to_date="+ strtime_to +"&page=1&per_page=1000"
	payload={}
	headers = {
	  'Authorization': 'Bearer '+ str(tokopedia_setting.token)
	}
	response = requests.request("GET", url, headers=headers, data=payload)
	datao = json.loads(response.text)

	for x in datao['data']:
		get_order = frappe.get_value("Marketplace Order",{"name" : str(x['order_id'])}, "name")
		urlsave = "https://fs.tokopedia.net/v2/fs/"+str(tokopedia_setting.app_id)+"/order?order_id=" + str(x['order_id'])
		payloadssave={}
		headers = {
  			'Authorization': 'Bearer '+ str(tokopedia_setting.token)
		}
		...
		if get_order :
			doc = frappe.get_doc("Marketplace Order",get_order)
			status = datasave['data']['order_status']
			doc.batas_kirim = datasave['data']['order_info']['delivered_age_day']
			doc.no_resi = datasave['data']['order_info']['shipping_info']['awb']
			if status == 0:
			...
			doc.no_referensi = str(x['order_id'])
			doc.marketplace = "Tokopedia"
			doc.shop_name = str(tokopedia_setting.shop_name)
			doc.tanggal = today
			...
			doc.batas_kirim = datasave['data']['order_info']['delivered_age_day']
			doc.id_pelanggan = datasave['data']['buyer_info']['buyer_id']
			doc.pelanggan = datasave['data']['buyer_info']['buyer_fullname']
			# doc.no_faktur = datasave['data']['payment_info']['payment_ref_num']
			doc.nama_penerima = datasave['data']['order_info']['destination']['receiver_name']
			...
			doc.alamat_penerima = alamat
			doc.telp_penerima = datasave['data']['order_info']['destination']['receiver_phone']
			doc.kota_penerima = datasave['data']['order_info']['destination']['address_city']
			...
			for i in datasave['data']['order_info']['order_detail']:
				...
				row.item_name = i['product_name']
				row.item_sku = i['sku']
				row.qty = int(i['quantity'])
				row.harga= i['product_price']
				row.diskon = int(i['product_price']) - int(i['subtotal_price'])
				row.total = int(i['quantity']) * int(i['subtotal_price'])
			...
			doc.total_berat = sum(total_weight)
			doc.sub_total = datasave['data']['item_price']
			doc.diskon = datasave['data']['promo_order_detail']['total_discount']
			doc.diskon_lainnya = datasave['data']['promo_order_detail']['total_cashback']
			doc.ongkos_kirim = datasave['data']['order_info']['shipping_info']['shipping_price'] 

			doc.asuransi = sum(total_asuransi)
			doc.potongan_biaya = datasave['data']['promo_order_detail']['total_discount_shipping']
			total_item = datasave['data']['item_price']
			total_diskon = datasave['data']['promo_order_detail']['total_discount']
			total_diskon_lainnya = datasave['data']['promo_order_detail']['total_cashback']
			total_ongkos = datasave['data']['order_info']['shipping_info']['shipping_price']
			totat_jumlah_asuransi = sum(total_asuransi)
			total_potongan_kirim = datasave['data']['promo_order_detail']['total_discount_shipping']
			doc.total = total_item - (total_diskon + total_diskon_lainnya) + total_ongkos + totat_jumlah_asuransi - total_potongan_kirim
			# for h in datasave['data']['order_info']['order_history']:
			# 	if i['hist_status_code'] == 500:
...
@frappe.whitelist()
def enqueue_tokopedia_order_manual():
	# frappe.msgprint("coba")
	date="2021-06-23"
	store_setting="MAMAM SHOP"
	# check setting
	tokopedia_setting = frappe.get_doc("Tokopedia Store Setting", store_setting)
...
def get_cat_manual2():
	...
	# child 1
	# for i in data[27]['child']:
	# 	doc = frappe.new_doc('Child Tokopedia')
	# 	doc.id_parent = data[27]['id']
	# 	doc.id = i['id']
...
def gen_token():
	...
	for i in datas:
		...
		get_token.flags.ignore_permissions = True
		get_token.save()
...
# 		strtime_to = str(time_to)
# 		url = "https://fs.tokopedia.net/v1/fs/"+str(tokopedia_setting.app_id)+"/fulfillment_order"
# 		#url = "https://fs.tokopedia.net/v2/order/list?fs_id="+str(tokopedia_setting.app_id)+"&shop_id="+i['shop_id']+"&from_date="+ strtime_from +"&to_date="+ strtime_to +"&page=1&per_page=10"
# 		payload={}
# 		headers = {
# 		  'Authorization': 'Bearer '+ str(tokopedia_setting.token)
# 		}
...
# 			get_order = frappe.get_value("Marketplace Order",{"name" : str(x['order']['order_id'])}, "name")
# 			urlsave = "https://fs.tokopedia.net/v2/fs/"+str(tokopedia_setting.app_id)+"/order?order_id=" + str(x['order']['order_id'])
# 			payloadssave={}
# 			headers = {
# 	  			'Authorization': 'Bearer '+ str(tokopedia_setting.token)
# 			}
...
# 				item = frappe.get_list("Marketplace Order Item",{"parent" : get_order},"parent")
...
def get_product():
	...
	for i in data_p['data']:
		get_p = frappe.get_value("Item",{"item_code" : i['sku']}, "item_code")
		if get_p:
			...
			doc.id_etalase = detail_p['data'][0]['menu']['id']
...
def update_price_stock():
	...
	for i in data_i:
		...
		url = "https://fs.tokopedia.net/inventory/v1/fs/"+datas[0]['app_id']+"/price/update?shop_id="+datas[0]['shop_id']
		payload = json.dumps([
  			{
    			"sku": i['item_code'],
    			"new_price": int(price)
	...
	for j in data_i:
		...
		url = "https://fs.tokopedia.net/inventory/v1/fs/"+datas[0]['app_id']+"/stock/update?shop_id="+datas[0]['shop_id']
		payload = json.dumps([
  			{
    			"sku": j['item_code'],
    			"new_stock": int(stock)
